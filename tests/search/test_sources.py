...
from eventfully.search.sources import kulturloewen, zuerichunbezahlbar
...
@pytest.mark.parametrize(
    "post_process_function,web_link,required_fields",
    [
        (
            zuerichunbezahlbar.post_process,
            "https://www.zuerichunbezahlbar.ch/events/kultur-nachtleben/film/2024-04-25-cinelowenbraukunst-screening/",
            ["description", "address", "operator_web_link"],
        ),
        (
...
def test_post_processing(
    post_process_function: Callable, web_link: str, required_fields: list[str]
):
    raw_event = schemas.Event(
        web_link=web_link, start_time=datetime.now(), end_time=datetime.now(), source=""
    )
